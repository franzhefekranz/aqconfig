...
# _ = gettext.gettext
# _ = lng.gettext
gettext.install('aqconfig_support', 'locales')
...
if cfg.is_file():
    ...
    if language == 'de':
        lng1 = gettext.translation('aqconfig_support',\
                                   localedir='locales',\
                                   languages=['de'])
        lng1.install()
        _ = lng1.gettext
    elif language == 'en':
        lng2 = gettext.translation('aqconfig_support',\
                                   localedir='locales',\
                                   languages=['en'])
        lng2.install()
        _ = lng2.gettext
    elif language == 'fr':
        lng3 = gettext.translation('aqconfig_support', \
                                   localedir='locales', \
                                   languages=['fr'])
        lng.install()
        _ = lng3.gettext
...
def set_Tk_var():
    ...
    global boolContextHelp
    boolContextHelp = BooleanVar()   
...
def get_ini():
    ...
    if cfg.is_file():
        ...
        language = (config['aqconfig'] ['language'])
        de = gettext.translation('aqconfig_support',\
                                  localedir='locales',\
                                  languages=['de'])
        de.install()
        _ = de.gettext
        aqconfig.set_lng(language)
        GenAboutBox.set_lng(language)
        lang_support.set_lng(language)
...
def open_context(anchor):
    """
    ############################################################################
    # open browser at anchor in html file 
    ############################################################################
    """
    url = str(Path(os.getcwd() + '/docs/en/index.html' + '#' + anchor))
    url = url.format(pathname2url(os.path.abspath(url)))
...
def OnMnuFileNew():
    ...
    getContext = boolContextHelp.get()
    if getContext == True:
        open_context('values')
...
def OnMnuFileSave():
    ...
    if getContext == True:
        ...
        global strMnuFileOpen
        if (strMnuFileOpen.get() == ''):
            OnMnuFileSaveAs()
        else:
            save_config_file(strMnuFileOpen.get())
...
def OnMnuSaveBat():
    ...
    if getContext == True:
        ...
        if (strPathToExe.get() == ''):
            messagebox.showinfo(_('Save *.bat'),_('path to aqserver.exe not set'))
            return
        elif (strMnuFileOpen.get() == ''):
            messagebox.showinfo(_('Save *.bat'),\
                                _('no config-file, save as .. or open config file'))
            return
        else:
...
def OnMnuHelpContents():
    ...
    ############################################################################
    """
    url = str(Path(os.getcwd() + '/docs/en/index.html'))
    url = url.format(pathname2url(os.path.abspath(url)))
    webbrowser.open_new(url)
#-------------------------------------------------------------------------------   
